...
import { CreateItemOrderDto, ItemDto } from "../../../api-services"

export const totalQuantity = (data: IFormInputsForCreateOrderInterface) => {
   return Number(
      Number(
...
export const totalDiscount = (data: IFormInputsForCreateOrderInterface) => {
   return Number(
      Number(
         data.itemDetails.reduce((acc, curr) => {
            if (
...
export const totalTax = (data: IFormInputsForCreateOrderInterface) => {
...
export const totalCost = (data: IFormInputsForCreateOrderInterface) => {
...
export const orderTax = (data: IFormInputsForCreateOrderInterface) => {
   return Number(
      Number(
...
export const grandTotal = (data: IFormInputsForCreateOrderInterface) => {
   return Number(
      Number(
         totalCost(data) +
            totalTax(data) -
...
export const itemDetailsFormatted = (
   data: IFormInputsForCreateOrderInterface,
) => {
   const formattedItems: CreateItemOrderDto[] = data.itemDetails.map((item) => {
      ...
      return {
         ...
         unitCostNet: Number(item.itemCost),
      ...
      if (item.itemBatches.length > 0) {
         itemBatches = item.itemBatches.map((itemBatch) => {
            ...
            const value = `${itemBatch.itemBatchId.toString()}-${item.itemId.toString()}-${
               ...
               item.taxMethod
            }-${item.unitCost.unitId.toString()}-${item.unitPrice.unitId.toString()}-${
               item.price
            }`
            return { label, value }
         })
